...
public class Controle_Personnage_Intro : MonoBehaviour {
		...
		void Update () {
				...
				//récupération de l'état du zoom de la caméra
				Finfade = raster.GetComponent<Titre_UI_Intro_Zoom> ().finfade;
				...
				if ((Input.GetButton ("Haut") & (!bougepas) & (Finfade)) | ((Input.GetAxisRaw ("VerticalJ") == -1) & (!bougepas)) & (Finfade)) {
						devant = true;
				} else
						devant = false;

				if ((Input.GetButton ("Bas") & (!bougepas) & (Finfade)) | ((Input.GetAxisRaw ("VerticalJ") == 1) & (!bougepas)) & (Finfade)) {
						derriere = true;
				} else
						derriere = false;

				if ((Input.GetButton ("Gauche") & (!bougepas) & (Finfade)) | ((Input.GetAxisRaw ("HorizontalJ") == -1) & (!bougepas)) & (Finfade)) {
						gauche = true;
				} else
						gauche = false;

				if ((Input.GetButton ("Droite") & (!bougepas) & (Finfade)) | ((Input.GetAxisRaw ("HorizontalJ") == 1) & (!bougepas)) & (Finfade)) {
						droite = true;
				} else
						droite = false;
				...
				if ((devant) & (!gauche) & (!droite) & (!derriere)
						...
						foreach (GameObject kub in kubs) {
								...
								if ((posfuture.x + (transform.localScale.x * 0.5F) >= kub.transform.position.x - (kub.transform.lossyScale.x * 0.5F))
										...
										& (posfuture.y + (transform.localScale.y * 0.5F) >= kub.transform.position.y - (kub.transform.lossyScale.y * 0.5F))
										& (posfuture.y - (transform.localScale.y * 0.5F) <= kub.transform.position.y + (kub.transform.lossyScale.y * 0.5F))
				...
				foreach (GameObject kub in kubs) {
						...
						if ((distsaut <= 0.5F) & (Mathf.RoundToInt (kub.transform.position.y) == persoy) & (kub.tag == "Kubs")) {
