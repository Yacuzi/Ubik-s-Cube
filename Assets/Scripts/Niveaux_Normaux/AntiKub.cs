	...
	void Start ()
	{
		/*
						...
						for (int i = 0; i < antikub.Length; i++){
								AntiLongueur[i] = char.Parse((antikub[i].name)[1]);
								AntiHauteur[i] = char.Parse((antikub[i].name)[3]);
								AntiLargeur[i] = char.Parse((antikub[i].name)[5]);
						}

						for (int i = 0; i < 10; i++) {
								Debug.Log (AntiLongueur [i] + " " + AntiLargeur [i] + " " + AntiHauteur [i]);
